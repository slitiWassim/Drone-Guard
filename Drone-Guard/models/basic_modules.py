...
class UP(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, stride=2):
         ...
         self.up = nn.ConvTranspose2d(in_channels=input_channels, out_channels=input_channels,
                                                kernel_size=kernel_size, stride=stride, padding=padding,
                                                output_padding=output_padding, bias=False)
         self.conv = ConvBnRelu(in_planes=2*input_channels,out_planes=output_channels,kernel_size=1)
    ...
    def __init__(self, input_channels, output_channels, kernel_size, stride=2):
        ...
        if stride != 2:     # ConvTranspose2d with factor = 4
            ...
            if kernel_size == 4:
                ...
                padding = 0
                output_padding = 0
        self.ConvTranspose = nn.ConvTranspose2d(in_channels=input_channels, out_channels=input_channels,
                                                kernel_size=kernel_size, stride=stride, padding=padding,
                                                output_padding=output_padding, bias=False)
        self.conv= ConvBn(in_planes=input_channels,out_planes=output_channels,kernel_size=1)
        #self.bn = nn.BatchNorm2d(output_channels, momentum=0.1)
        #self.relu = nn.ReLU(inplace=True)
